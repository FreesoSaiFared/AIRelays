...
six worker tabs                         orchestrator tab
     ...
     +---------------- Brave CDP -------------+
            ...
            persistent Windows process
               |                    |
       process interference     control HTTP
            guard              127.0.0.1 only
                                     |
                              session-farm-mcp.mjs
                                     |
...
6. that exact assistant output has not already been continued;
...
The assistant output hash is recorded before submission, so the same completed turn is never continued twice.
...
Loopback addresses in this subsystem are same-host IPC only. Inter-host access is deliberately not implemented through `127.0.0.1`; use the user's Tailscale/MagicDNS or another explicit transport when a remote host is involved.
...
5. the same assistant-output hash is not continued a second time;
6. a busy or paused worker receives none;
7. the process guard reports exactly which matching processes it terminated;
8. the orchestrator can pause/resume/nudge one worker through a `FARM_CONTROL` envelope;
9. no browser extension is required for any of the above.
